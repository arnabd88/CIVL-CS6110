...
domain. Note that if a range expression can be used as \texttt{dom} here, which will be 
...
Note that if a range expression can be used as \texttt{dom} here, which will be 
automatically converted to one dimensional domain. For example, 
 \begin{verbatim}
  $for (int i1, ..., in : 0 .. 10) S
\end{verbatim}
