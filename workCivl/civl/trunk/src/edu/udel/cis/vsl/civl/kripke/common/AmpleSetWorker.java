...
public class AmpleSetWorker {
	...
	Pair<Boolean, Set<ProcessState>> ampleProcesses() {
		...
		if (activeProcesses.cardinality() <= 1) {
			...
			//System.out.println("\nAmpleProcessIds = " + ampleProcessIDs + "\n");
		   //System.out.println("AmpleProcessIds-Length = " + ampleProcessIDs.length());
		for (int pid = 0; pid < ampleProcessIDs.length(); pid++) {
		   // System.out.println("\nPID before nextSetBit = " + pid) ;
			pid = ampleProcessIDs.nextSetBit(pid);
		   // System.out.println("PID after nextSetBit = " + pid + "\n") ;
